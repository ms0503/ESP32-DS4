...
public:
    ...
    void set_bit(uint16_t type, bool bit);

    void set_sticks(int8_t rx, int8_t ry, int8_t lx, int8_t ly);

    void set_triggers(uint8_t r2, uint8_t l2);
...
private:
    ...
    static std::uint8_t get_checksum(std::array<std::uint8_t, 10> raw);
};
